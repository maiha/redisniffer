class Program
  def initialize(@pcap : Pcap::Capture, @ports : Set(Int32), @blacklist : Hash(String, Bool), @flusher : Flusher, @interval : Time::Span = 1.second, @verbose : Bool = false)
  end
  ...
  def run
    ...
    @pcap.loop do |pkt|
      ...
      if Time.now > last_flushed_at + @interval
        flusher.flush(hash)
        hash.clear
        last_flushed_at = Time.now
      end
